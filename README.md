...
## Tasks Completed
**System Orientation**
**Q1:** **Print your current directory, your username, and full OS and kernel details — three separate commands. What does each command tell you? Why does a DevSecOps engineer run these three things first when SSHing into an unknown server — especially regarding kernel CVEs?**
...
**Root Directory Exploration & FHS**
...
FHS was developed and adopted as the standard for Linux to eradicate the menace of dumping files anywhere as deemed necessary by the creators before the mid-1990s. FHS addresses the three benefits:
...
**Listing Home Directory & Hidden Files**
...
- ls -al: while ls l displays all files, directories, and other details, -a includes all the hidden ones – special entries “.” Current directory and “..” parent directory (.profile, .ssh, .local as shown in the picture). The command also shows detailed information such as:
...
produces:
a b c
In this task, brace expansion allowed me to create several directories with one command:
mkdir -p ~/projects/cyphercore/{configs,reports,logs/{access,errors,archive}}
...
However, using brace expansion is much faster, requires less typing, and reduces the chance of making mistakes. For creating multiple related directories, it is generally the preferred approach.

**Bulk File Creation with Timestamps**
...
For example, a deployment script can run:
touch app.conf
...
**Permission Breakdown & Human-Readable Formats**
...
Without -h, sizes are displayed in bytes:
ls -l
Example:
1024
1048576
...
**Top-to-Bottom vs. Bottom-to-Top Reversals**
...
**Heredoc Implementations & Forensic Artifacts**
...
**more command**: press enter key to navigate line by line; space key to navigate page by page; PgDn/PgUp to navigate page down/page up. q key to quit.
...
I would use: **less** command because it can efficiently handle large files without requiring the entire file to be loaded into memory.
For troubleshooting recent events, I might first use: **tail -n 50 [filename]**, and then use less command for deeper investigation.
...
**Directory Destruction Guardrails**
...
<img width="940" height="195" alt="image" src="https://github.com/user-attachments/assets/8b60f65d-45e1-4652-9855-5c8938d28f29" />
...
**Hard Links and Inode Integrity**
...
**Symbolic Links and Broken Pipelines**
...
Command: ln -s ~/projects/cyphercore/configs/app.conf ~/projects/cyphercore/app_config_link
...
Command: ls -l ~/projects/cyphercore
...
Command: rm ~/projects/cyphercore/configs/app.conf
...
**Soft link:**
Uses a soft link to point to a shared configuration file across different servers or containers in a DevSecOps pipeline. For example, symlinking /etc/app/config.yaml to the latest version in /opt/deploy/releases/current/config.yaml allows rolling updates without modifying scripts that reference the symlink.
...
Command: ls ~/projects/cyphercore/logs/ > ~/projects/cyphercore/reports/stdout_output.txt 
...
Command: ls ~/projects/cyphercore/fakefile_path/ 2> ~/projects/cyphercore/reports/stderr_output.txt
...
**Formatted Reports & Expansion Controls**
...
Command: tree ~/projects/cyphercore (alternatively – find ~/projects/cyphercore | sort)
